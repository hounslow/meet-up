...
public class Schedule {
    ...
    public boolean hasBreakTimeOfDay(String timeOfDay, String dayOfWeek) {
        List<String> breakTimes = new ArrayList<>(this.getStartTimesOfBreaks(dayOfWeek));
        int timeMinutes = calculateMinutesIntoDay(timeOfDay.concat(":00"));
        boolean inRange = false;
        for (int i = 0; i < breakTimes.size(); i++) {
            int breakTimeMins = calculateMinutesIntoDay(breakTimes.get(i));
            if(breakTimeMins <= timeMinutes+60){
                inRange = true;
            }
        }
